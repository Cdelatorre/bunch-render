                    ...
                    <tbody>
                        @forelse ($winners as $winner)
                            <tr>
                                ...
                                <td data-label="Request Price">{{ showAmount(@$winner->getRequestUser->price, 2) }}</td>
                                <td data-label="Request Time">
                                    {{ showDateTime($winner->getRequestUser->created_at, 'd M, Y H:s:ia') }}
                                </td>
                                @php
                                    $image = ($winner->getProduct->productImages)[0];
                                    $mainImageUrl = getImage(getFilePath('product') . '/'.@$image->path .'/' .@$image->image);
                                @endphp
                                <td data-label="View">
                                    <div class="button--group">
                                        <a title="@lang('Details')" href="javascript:void(0)" class="btn btn--sm btn--base detailsBtn border-none" data-owner="{{ $winner->getProduct->user ? $winner->getProduct->user->fullname : 'Administrator' }}" data-title="{{$winner->getProduct->title}}" data-price="{{ showAmount($winner->getProduct->price,2) }}" data-product="{{$winner->getProduct}}" data-time="{{showDateTime($winner->getRequestUser->created_at, 'd M, Y H:s:ia')}}" data-image="{{ $mainImageUrl }}">
