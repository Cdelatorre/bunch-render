...
<body>
    ...
    @php
        ...
        $errorContent = getContent('error_404.content', true);
    @endphp
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="breadcumb__wrapper">
                        <h2 class="breadcumb__title">404 @lang('Error')  </h2>
                        <ul class="breadcumb__list">
                            ...
                            <li class="breadcumb__item"> <span class="breadcumb__item-text"> 404 @lang('Error')  </span> </li>
                        </ul>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-lg-6">
                        <div class="error-wrap text-center">
                            <div class="error__text">
                                <span>{{__(@$errorContent->data_values->heading)}}....</span>
                            </div>
                            <h2 class="error-wrap__title">{{__(@$errorContent->data_values->subheading)}}</h2>
                            <p class="error-wrap__desc">{{__(@$errorContent->data_values->description)}}</p>

                        </div>
                    </div>
                    <div class="col-lg-6">
                       <div class="error-thumb text-center">
                       @php($errorContent = getContent('error_404.content', true))

                            {{-- Evita el fatal si el bloque no existe --}}
                            @php($errorVals = $errorContent ? $errorContent->data_values : (object)[])

                            <span>{{ __($errorVals->heading ?? 'Page not found') }}</span>

                            @if(($errorVals->has_image ?? false))
                                <img src="{{ getImage(getFilePath('error_404').'/'.$errorVals->image) }}">
                            @else
                                <img src="{{ assetTemplate() }}/images/error-img.png">
                            @endif
                       </div>
                    </div>
                </div>
